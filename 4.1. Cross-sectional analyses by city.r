...
vars <- c(
  ...
  "ngo_condom_rec_bin", "ngo_syringe_12m_bin", "hiv_test_rslt",
...
compute_results_ngo <- function(df, vars) {
  ...
  for (v in vars) {
    ...
    dat <- df %>%
      select(ngo_client_lifetime_3cat, years_in_sw_3cat, all_of(v)) %>%
      filter(
    ...
    model <- try(glm(as.formula(paste("ngo_client_lifetime_3cat ~", rhs)),
...
compute_results_ngo_condom <- function(df, vars) {
  ...
  for (v in vars) {
    ...
    dat <- df %>%
      select(ngo_condom_rec_bin, years_in_sw_3cat, all_of(v)) %>%
      filter(
        !is.na(ngo_condom_rec_bin),
        !is.na(.data[[v]]),
        !is.na(years_in_sw_3cat)
      )

    if (nrow(dat) == 0 ||
        length(unique(dat$ngo_condom_rec_bin)) < 2 ||
        length(unique(dat[[v]])) < 2) next

    rhs <- paste("years_in_sw_3cat", v, sep = " + ")

    model <- try(glm(as.formula(paste("ngo_condom_rec_bin ~", rhs)),
                     data = dat, family = binomial), silent = TRUE)
    ...
    for (lev in levels_var) {
      ...
      cases <- sum(sub$ngo_condom_rec_bin, na.rm = TRUE)
      pct <- round(cases / n * 100, 1)
      count_pct <- sprintf("%d (%.1f%%)", cases, pct)
...
compute_results <- function(df, vars) {
  ...
  for (v in vars) {
    ...
    dat <- df %>%
      select(condom_access_12m_bin, years_in_sw_3cat, year, all_of(v)) %>%
      filter(
    ...
    model <- try(
      glmer(as.formula(paste("condom_access_12m_bin ~", rhs, "+ (1 | year)")),
            data = dat,
            family = binomial(link = "logit"),
            control = glmerControl(optimizer = "bobyqa")),
      silent = TRUE
...
for (cty in levels(sw_combined_clean$city)) {
  ...
  write_xlsx(out, file.path("outputs_by_city", cty, "condom_access_results.xlsx"))
}
